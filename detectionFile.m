...
%    10^6*(P_ecg_aligned(i))/(XML.TestInfo.ECGSampleBase * 1000);       Den gamle metode
    sum_p_loop(nr) = sum_p_loop(nr) + (trapz(P_ecg_aligned(i-1:i))/XML.TestInfo.ECGSampleBase)*1000;
    end 
...
%    sum_p_inv_loop(nr) = sum_p_inv_loop(nr) + 10^6*(P_ecg_aligned(i))/(XML.TestInfo.ECGSampleBase * 1000);    % den gamle metode    
    sum_p_inv_loop(nr) = sum_p_inv_loop(nr) + (trapz(P_ecg_aligned(i-1:i))/XML.TestInfo.ECGSampleBase)*1000;
    end  
end
